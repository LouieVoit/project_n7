...
%------------------------------------------------------------------------
function [ITS,DELT_PHI,seg] = region_seg_stop_area_weight(I,init_mask,max_its,alpha,display,tol,G)
  ...
  while its <= max_its && ~stop  % Note: no automatic convergence test
    ...
    dphidt = F./max(abs(F)) + alpha*curvature + 300*Fg/a;  % gradient descent to minimize energy
   
     %dphidt = Fg/a;
%size(dphidt)
    
    %-- maintain the CFL condition
    dt = abs(.45/(max(dphidt)+eps));
