...
%------------------------------------------------------------------------
function [ITS,DELT_PHI,seg] = region_seg_stop_curve_weight(I,init_mask,max_its,alpha,display,tol,Gs,Gl)
...
%    Fgs = Gs(idx);
    Fgl = get_curve_weight(phi,Gl,idx);
    
    % dphidt = F./max(abs(F)) + alpha*curvature - Fg;  % gradient descent to minimize energy
    % dphidt = Fgs/a;
    dphidt = Fgl;
...
function stop = isvarlow(DELT_PHI, ITS, tol)
    ...
    if nb_its < 20
        ...
        y=filter(b,a,D);
        % Critère d'arrêt : sur les 20 dernières itérations N(dphi) est resté
        % autour de 0.
...
%-- compute curve weight term along SDF
function c_weight = get_curve_weight(phi,u,idx)
    [dimy, dimx] = size(phi);        
    ...
    %-- get central derivates of u at x,y
    u_x  = -phi(idlt)+phi(idrt);
    u_y  = -phi(iddn)+phi(idup);
    
    %-- curve weight term
    c_weight = u_x.*phi_x + u_y.*phi_y;   
